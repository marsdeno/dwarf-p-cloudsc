...
#ifdef CLOUDSC_GPU_SCC_FIELD_BLOCKED
USE CLOUDSC_FIELD_STATE_MOD, ONLY: CLOUDSC_FIELD_STATE
USE CLOUDSC_DRIVER_GPU_SCC_FIELD_BLOCKED_MOD, ONLY: CLOUDSC_DRIVER_GPU_SCC_FIELD_BLOCKED
USE FIELD_DEFAULTS_MODULE, ONLY: INIT_PINNED_VALUE
#endif

#ifdef CLOUDSC_GPU_SCC_FIELD_ASYNC
USE CLOUDSC_FIELD_STATE_MOD, ONLY: CLOUDSC_FIELD_STATE
USE CLOUDSC_DRIVER_GPU_SCC_FIELD_ASYNC_MOD, ONLY: CLOUDSC_DRIVER_GPU_SCC_FIELD_ASYNC
USE FIELD_DEFAULTS_MODULE, ONLY: INIT_PINNED_VALUE
#endif
...
CHARACTER(LEN=20) :: NQUEUES_CHARS
CHARACTER(LEN=5)  :: BLOCKING_CHUNK_SIZE_CHARS
LOGICAL :: USE_PACKED
...
#if defined(CLOUDSC_GPU_SCC_FIELD) || defined(CLOUDSC_GPU_SCC_FIELD_BLOCKED) || \
    defined(CLOUDSC_GPU_SCC_FIELD_ASYNC)
...
#elif defined(CLOUDSC_GPU_SCC_FIELD_BLOCKED)
! Use Field API pinned memory capabilities for
CALL GET_ENVIRONMENT_VARIABLE('CLOUDSC_FIELD_API_PINNED', PINNED_FIELD_MEMORY)
INIT_PINNED_VALUE = TRIM(PINNED_FIELD_MEMORY) == 'ON' .OR. TRIM(PINNED_FIELD_MEMORY) == '1'
! Create a global memory state from serialized input data
CALL GLOBAL_STATE%LOAD(NPROMA, NGPTOT, NGPTOTG, USE_PACKED=.FALSE.)
! Load block size from env.variable if present
CALL GET_ENVIRONMENT_VARIABLE('CLOUDSC_BLOCKING_CHUNK_SIZE', BLOCKING_CHUNK_SIZE_CHARS, STATUS=ENV_STATUS)
IF (ENV_STATUS == 0) THEN
  READ( BLOCKING_CHUNK_SIZE_CHARS, * ) BLOCKING_CHUNK_SIZE
END IF
#elif defined(CLOUDSC_GPU_SCC_FIELD_ASYNC)
! Use Field API pinned memory capabilities for
CALL GET_ENVIRONMENT_VARIABLE('CLOUDSC_FIELD_API_PINNED', PINNED_FIELD_MEMORY)
INIT_PINNED_VALUE = TRIM(PINNED_FIELD_MEMORY) == 'ON' .OR. TRIM(PINNED_FIELD_MEMORY) == '1'
! Create a global memory state from serialized input data
CALL GLOBAL_STATE%LOAD(NPROMA, NGPTOT, NGPTOTG, USE_PACKED=.FALSE.)
! Load number of queues from env. variable if present
CALL GET_ENVIRONMENT_VARIABLE('CLOUDSC_ASYNC_NQUEUES', NQUEUES_CHARS, STATUS=ENV_STATUS)
IF (ENV_STATUS == 0) THEN
...
#ifdef CLOUDSC_GPU_SCC_FIELD_BLOCKED
print *, 'Executing CLOUDSC-GPU, "SCC-FIELD-BLOCKED", BLOCKING_CHUNK_SIZE,', BLOCKING_CHUNK_SIZE, 'PINNED FIELDS', INIT_PINNED_VALUE

! Call the driver to perform the parallel loop over our kernel
CALL CLOUDSC_DRIVER_GPU_SCC_FIELD_BLOCKED( &
     & NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, GLOBAL_STATE%NBLOCKS, NGPTOTG, &
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%AUX, GLOBAL_STATE%FLUX, &
     & GLOBAL_STATE%TENDENCY_TMP, GLOBAL_STATE%TENDENCY_LOC, BLOCKING_CHUNK_SIZE)
#endif

#ifdef CLOUDSC_GPU_SCC_FIELD_ASYNC
print *, 'Executing CLOUDSC-GPU, "SCC-FIELD-ASYNC", NQUEUES', NQUEUES, ", BLOCKING_CHUNK_SIZE", BLOCKING_CHUNK_SIZE, ", PINNED FIELDS", INIT_PINNED_VALUE

! Call the driver to perform the parallel loop over our kernel
CALL CLOUDSC_DRIVER_GPU_SCC_FIELD_ASYNC( &
     & NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, GLOBAL_STATE%NBLOCKS, NGPTOTG, &
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%AUX, GLOBAL_STATE%FLUX, &
     & GLOBAL_STATE%TENDENCY_TMP, GLOBAL_STATE%TENDENCY_LOC, BLOCKING_CHUNK_SIZE, NQUEUES)
#endif
