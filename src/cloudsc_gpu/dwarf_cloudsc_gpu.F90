...
CHARACTER(LEN=5)  :: BLOCK_SIZE_CHARS
LOGICAL :: USE_PACKED
...
INTEGER(KIND=JPIM) :: BLOCK_SIZE = 512 ! Default block size for blocked and async F-API variants
...
CALL GET_ENVIRONMENT_VARIABLE('CLOUDSC_BLOCK_SIZE', BLOCK_SIZE_CHARS, STATUS=ENV_STATUS)
IF (ENV_STATUS == 0) THEN
  READ( BLOCK_SIZE_CHARS, * ) BLOCK_SIZE
END IF
...
#ifdef CLOUDSC_GPU_SCC_FIELD_BLOCKED
print *, 'Executing CLOUDSC-GPU, "SCC-FIELD-BLOCKED", BLOCK_SIZE,', BLOCK_SIZE, 'PINNED FIELDS', INIT_PINNED_VALUE
...
CALL CLOUDSC_DRIVER_GPU_SCC_FIELD_BLOCKED( &
     ...
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%AUX, GLOBAL_STATE%FLUX, &
     & GLOBAL_STATE%TENDENCY_TMP, GLOBAL_STATE%TENDENCY_LOC, BLOCK_SIZE)
#endif

#ifdef CLOUDSC_GPU_SCC_FIELD_ASYNC
print *, 'Executing CLOUDSC-GPU, "SCC-FIELD-ASYNC", NQUEUES', NQUEUES, ", BLOCK_SIZE", BLOCK_SIZE, ", PINNED FIELDS", INIT_PINNED_VALUE
...
CALL CLOUDSC_DRIVER_GPU_SCC_FIELD_ASYNC( &
     ...
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%AUX, GLOBAL_STATE%FLUX, &
     & GLOBAL_STATE%TENDENCY_TMP, GLOBAL_STATE%TENDENCY_LOC, BLOCK_SIZE, NQUEUES)
#endif
