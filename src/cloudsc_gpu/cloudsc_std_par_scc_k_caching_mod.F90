...
MODULE CLOUDSC_STD_PAR_SCC_K_CACHING_MOD
  
  CONTAINS
  
  SUBROUTINE CLOUDSC_SCC_K_CACHING (KIDIA, NGPTOT, NPROMA, KFDIA, KLON, KLEV, PTSPHY, PT, PQ, TENDENCY_TMP, TENDENCY_LOC, PVFA,  &
  & PVFL, PVFI, PDYNA, PDYNL, PDYNI, PHRSW, PHRLW, PVERVEL, PAP, PAPH, PLSM, LDCUM, KTYPE, PLU, PLUDE, PSNDE, PMFU, PMFD, PA,  &
  & PCLV, PSUPSAT, PLCRIT_AER, PICRIT_AER, PRE_ICE, PCCN, PNICE, PCOVPTOT, PRAINFRAC_TOPRFZ, PFSQLF, PFSQIF, PFCQNNG, PFCQLNG,  &
  & PFSQRF, PFSQSF, PFCQRNG, PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP, NGPBLKS)
    ...
    ! USE YOMCST_CUF, ONLY: RG => RG_D, RD => RD_D, RCPD => RCPD_D, RETV => RETV_D, RLVTT => RLVTT_D, RLSTT => RLSTT_D,  &
    ! & RLMLT => RLMLT_D, RTT => RTT_D, RV => RV_D
    ! USE YOETHF_CUF, ONLY: R2ES => R2ES_D, R3LES => R3LES_D, R3IES => R3IES_D, R4LES => R4LES_D, R4IES => R4IES_D,  &
    ! & R5LES => R5LES_D, R5IES => R5IES_D, R5ALVCP => R5ALVCP_D, R5ALSCP => R5ALSCP_D, RALVDCP => RALVDCP_D,  &
    ...
    REAL(KIND=JPRB), INTENT(IN) :: PLCRIT_AER(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(IN) :: PICRIT_AER(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(IN) :: PRE_ICE(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(IN) :: PCCN(KLON, KLEV, NGPBLKS)    ! liquid cloud condensation nuclei
    REAL(KIND=JPRB), INTENT(IN) :: PNICE(KLON, KLEV, NGPBLKS)    ! ice number concentration (cf. CCN)
    ...
    INTEGER(KIND=JPIM), VALUE, INTENT(IN) :: KFDIA, NPROMA, NGPTOT
    REAL(KIND=JPRB), VALUE, INTENT(IN) :: PTSPHY    ! Physics timestep
    REAL(KIND=JPRB), INTENT(IN) :: PT(KLON, KLEV, NGPBLKS)    ! T at start of callpar
    REAL(KIND=JPRB), INTENT(IN) :: PQ(KLON, KLEV, NGPBLKS)    ! Q at start of callpar
    REAL(KIND=JPRB), INTENT(IN) :: TENDENCY_TMP(KLON, KLEV, NCLV + 3, NGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: TENDENCY_LOC(KLON, KLEV, NCLV + 3, NGPBLKS)
    REAL(KIND=JPRB), INTENT(IN) :: PVFA(KLON, KLEV, NGPBLKS)    ! CC from VDF scheme
    REAL(KIND=JPRB), INTENT(IN) :: PVFL(KLON, KLEV, NGPBLKS)    ! Liq from VDF scheme
    REAL(KIND=JPRB), INTENT(IN) :: PVFI(KLON, KLEV, NGPBLKS)    ! Ice from VDF scheme
    REAL(KIND=JPRB), INTENT(IN) :: PDYNA(KLON, KLEV, NGPBLKS)    ! CC from Dynamics
    REAL(KIND=JPRB), INTENT(IN) :: PDYNL(KLON, KLEV, NGPBLKS)    ! Liq from Dynamics
    REAL(KIND=JPRB), INTENT(IN) :: PDYNI(KLON, KLEV, NGPBLKS)    ! Liq from Dynamics
    REAL(KIND=JPRB), INTENT(IN) :: PHRSW(KLON, KLEV, NGPBLKS)    ! Short-wave heating rate
    REAL(KIND=JPRB), INTENT(IN) :: PHRLW(KLON, KLEV, NGPBLKS)    ! Long-wave heating rate
    REAL(KIND=JPRB), INTENT(IN) :: PVERVEL(KLON, KLEV, NGPBLKS)    !Vertical velocity
    REAL(KIND=JPRB), INTENT(IN) :: PAP(KLON, KLEV, NGPBLKS)    ! Pressure on full levels
    REAL(KIND=JPRB), INTENT(IN) :: PAPH(KLON, KLEV + 1, NGPBLKS)    ! Pressure on half levels
    REAL(KIND=JPRB), INTENT(IN) :: PLSM(KLON, NGPBLKS)    ! Land fraction (0-1)
    LOGICAL, INTENT(IN) :: LDCUM(KLON, NGPBLKS)    ! Convection active
    INTEGER(KIND=JPIM), INTENT(IN) :: KTYPE(KLON, NGPBLKS)    ! Convection type 0,1,2
    REAL(KIND=JPRB), INTENT(IN) :: PLU(KLON, KLEV, NGPBLKS)    ! Conv. condensate
    REAL(KIND=JPRB), INTENT(INOUT) :: PLUDE(KLON, KLEV, NGPBLKS)    ! Conv. detrained water
    REAL(KIND=JPRB), INTENT(IN) :: PSNDE(KLON, KLEV, NGPBLKS)    ! Conv. detrained snow
    REAL(KIND=JPRB), INTENT(IN) :: PMFU(KLON, KLEV, NGPBLKS)    ! Conv. mass flux up
    REAL(KIND=JPRB), INTENT(IN) :: PMFD(KLON, KLEV, NGPBLKS)    ! Conv. mass flux down
    REAL(KIND=JPRB), INTENT(IN) :: PA(KLON, KLEV, NGPBLKS)    ! Original Cloud fraction (t)
    
    REAL(KIND=JPRB), INTENT(IN) :: PCLV(KLON, KLEV, NCLV, NGPBLKS)
    
    ! Supersat clipped at previous time level in SLTEND
    REAL(KIND=JPRB), INTENT(IN) :: PSUPSAT(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(OUT) :: PCOVPTOT(KLON, KLEV, NGPBLKS)    ! Precip fraction
    REAL(KIND=JPRB), INTENT(OUT) :: PRAINFRAC_TOPRFZ(KLON, NGPBLKS)
    ! Flux diagnostics for DDH budget
    REAL(KIND=JPRB), INTENT(OUT) :: PFSQLF(KLON, KLEV + 1, NGPBLKS)    ! Flux of liquid
    REAL(KIND=JPRB), INTENT(OUT) :: PFSQIF(KLON, KLEV + 1, NGPBLKS)    ! Flux of ice
    REAL(KIND=JPRB), INTENT(OUT) :: PFCQLNG(KLON, KLEV + 1, NGPBLKS)    ! -ve corr for liq
    REAL(KIND=JPRB), INTENT(OUT) :: PFCQNNG(KLON, KLEV + 1, NGPBLKS)    ! -ve corr for ice
    REAL(KIND=JPRB), INTENT(OUT) :: PFSQRF(KLON, KLEV + 1, NGPBLKS)    ! Flux diagnostics
    REAL(KIND=JPRB), INTENT(OUT) :: PFSQSF(KLON, KLEV + 1, NGPBLKS)    !    for DDH, generic
    REAL(KIND=JPRB), INTENT(OUT) :: PFCQRNG(KLON, KLEV + 1, NGPBLKS)    ! rain
    REAL(KIND=JPRB), INTENT(OUT) :: PFCQSNG(KLON, KLEV + 1, NGPBLKS)    ! snow
    REAL(KIND=JPRB), INTENT(OUT) :: PFSQLTUR(KLON, KLEV + 1, NGPBLKS)    ! liquid flux due to VDF
    REAL(KIND=JPRB), INTENT(OUT) :: PFSQITUR(KLON, KLEV + 1, NGPBLKS)    ! ice flux due to VDF
    REAL(KIND=JPRB), INTENT(OUT) :: PFPLSL(KLON, KLEV + 1, NGPBLKS)    ! liq+rain sedim flux
    REAL(KIND=JPRB), INTENT(OUT) :: PFPLSN(KLON, KLEV + 1, NGPBLKS)    ! ice+snow sedim flux
    REAL(KIND=JPRB), INTENT(OUT) :: PFHPSL(KLON, KLEV + 1, NGPBLKS)    ! Enthalpy flux for liq
    REAL(KIND=JPRB), INTENT(OUT) :: PFHPSN(KLON, KLEV + 1, NGPBLKS)    ! Enthalp flux for ice
    ...
    REAL(KIND=JPRB) :: ZLCOND1, ZLCOND2, ZLEVAPL, ZLEVAPI, ZRAINAUT, ZSNOWAUT, ZLIQCLD, ZICECLD
    ...
    REAL(KIND=JPRB) :: ZLIQFRAC
    REAL(KIND=JPRB) :: ZICEFRAC
    REAL(KIND=JPRB) :: ZQX(NCLV)
    REAL(KIND=JPRB) :: ZQX0(NCLV) 
    ...
    REAL(KIND=JPRB) :: ZPFPLSX(2, NCLV)  
    REAL(KIND=JPRB) :: ZLNEG(NCLV) 
    ...
    REAL(KIND=JPRB) :: ZRATIO(NCLV), ZSINKSUM(NCLV)
    ...
    INTEGER(KIND=JPIM), VALUE, INTENT(IN) :: NGPBLKS
    INTEGER(KIND=JPIM) :: WORKER, TMP_IBL, IBL
    INTEGER(KIND=JPIM), PARAMETER  :: NUM_WORKER = 8
...
#include "fccld.func.h"
    ...
    ! Fluxes:
    !--------
    PFSQLF(JL, 1, IBL) = 0.0_JPRB
    PFSQIF(JL, 1, IBL) = 0.0_JPRB
    PFSQRF(JL, 1, IBL) = 0.0_JPRB
    PFSQSF(JL, 1, IBL) = 0.0_JPRB
    PFCQLNG(JL, 1, IBL) = 0.0_JPRB
    PFCQNNG(JL, 1, IBL) = 0.0_JPRB
    PFCQRNG(JL, 1, IBL) = 0.0_JPRB      !rain
    PFCQSNG(JL, 1, IBL) = 0.0_JPRB      !snow
    ! fluxes due to turbulence
    PFSQLTUR(JL, 1, IBL) = 0.0_JPRB
    PFSQITUR(JL, 1, IBL) = 0.0_JPRB
    ...
    PRAINFRAC_TOPRFZ(JL, IBL) = 0.0_JPRB      ! rain fraction at top of refreezing layer
    ...
    IF (1<=JK .and. JK<=KLEV) THEN
      ZTP1(JK_I) = PT(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 1, IBL)
      ZQX(NCLDQV) = PQ(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 3, IBL)
      ZQX0(NCLDQV) = PQ(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 3, IBL)
      ZA(JK_I) = PA(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 2, IBL)
      ZAORIG = PA(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 2, IBL)
    ...
    IF (ZQX(NCLDQL) + ZQX(NCLDQI) < YRECLDP%RLMIN .or. ZA(JK_I) < YRECLDP%RAMIN) THEN
      ...
      TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + ZQADJ
      TENDENCY_LOC(JL, JK, 1, IBL) = TENDENCY_LOC(JL, JK, 1, IBL) - RALVDCP*ZQADJ
      ...
      TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + ZQADJ
      TENDENCY_LOC(JL, JK, 1, IBL) = TENDENCY_LOC(JL, JK, 1, IBL) - RALSDCP*ZQADJ
    ...
    DO JM=1,NCLV - 1
      IF (ZQX(JM) < YRECLDP%RLMIN) THEN
        ...
        ZQADJ = ZQX(JM)*ZQTMST
        TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + ZQADJ
        IF (IPHASE(JM) == 1)             TENDENCY_LOC(JL, JK, 1, IBL) = TENDENCY_LOC(JL, JK, 1, IBL) - RALVDCP*ZQADJ
        IF (IPHASE(JM) == 2)             TENDENCY_LOC(JL, JK, 1, IBL) = TENDENCY_LOC(JL, JK, 1, IBL) - RALSDCP*ZQADJ
    ...
    !----------------------------------------
    ZFOEALFA = (MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2))
    ZFOEEWMT = MIN(((R2ES*((MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2))*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES)) + (1.0_JPRB - (MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2)))*EXP((R3IES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4IES))))) / PAP(JL, JK, IBL), 0.5_JPRB)
    ZQSMIX = ZFOEEWMT
    ...
    !---------------------------------------------
    ZALFA = ((MAX(0.0_JPRB, SIGN(1.0, ZTP1(JK_I) - RTT))))
    ZFOEEW =  &
    & MIN((ZALFA*((R2ES*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES)))) + (1.0_JPRB - ZALFA)*((R2ES*EXP((R3IES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4IES))))) / PAP(JL, JK, IBL), 0.5_JPRB)
    ...
    !----------------------------------
    ZFOEELIQT = MIN(((R2ES*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES)))) / PAP(JL, JK, IBL), 0.5_JPRB)
    ZQSLIQ = ZFOEELIQT
    ...
    IF (YRECLDP%NCLDTOP<=JK .AND. JK<=KLEV) THEN
      ...
      ZDP = PAPH(JL, JK + 1, IBL) - PAPH(JL, JK, IBL)          ! dp
      ZGDP = RG / ZDP          ! g/dp
      ZRHO = PAP(JL, JK, IBL) / (RD*ZTP1(JK_I))          ! p/RT air density
      
      ZDTGDP = PTSPHY*ZGDP          ! dt g/dp
      ZRDTGDP = ZDP*(1.0_JPRB / (PTSPHY*RG))          ! 1/(dt g/dp)
      
      IF (JK > 1)         ZDTGDPF = PTSPHY*RG / (PAP(JL, JK, IBL) - PAP(JL, JK - 1, IBL))
      ...
      ZCORQSMIX = 1.0_JPRB + (((MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2)))*RALVDCP+(1.0_JPRB-((MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2))))*RALSDCP)*ZDQSMIXDT
      ...
      ZFOKOOP = ((MIN(RKOOP1 - RKOOP2*ZTP1(JK_I), (R2ES*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES)))*1.0_JPRB/(R2ES*EXP((R3IES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4IES))))))
      ...
      !-------------------------------------------------------
      IF (PSUPSAT(JL, JK, IBL) > ZEPSEC) THEN
        IF (ZTP1(JK_I) > YRECLDP%RTHOMO) THEN
          ! Turn supersaturation into liquid water
          ZSOLQA(NCLDQL, NCLDQL) = ZSOLQA(NCLDQL, NCLDQL) + PSUPSAT(JL, JK, IBL)
          ZPSUPSATSRCE(NCLDQL) = PSUPSAT(JL, JK, IBL)
          ! Add liquid to first guess for deposition term
          ZQXFG(NCLDQL) = ZQXFG(NCLDQL) + PSUPSAT(JL, JK, IBL)
          ! Store cloud budget diagnostics if required
        ELSE
          ! Turn supersaturation into ice water
          ZSOLQA(NCLDQI, NCLDQI) = ZSOLQA(NCLDQI, NCLDQI) + PSUPSAT(JL, JK, IBL)
          ZPSUPSATSRCE(NCLDQI) = PSUPSAT(JL, JK, IBL)
          ! Add ice to first guess for deposition term
          ZQXFG(NCLDQI) = ZQXFG(NCLDQI) + PSUPSAT(JL, JK, IBL)
          ! Store cloud budget diagnostics if required
        END IF
      ...
      IF (JK < KLEV .and. JK >= YRECLDP%NCLDTOP) THEN
        ...
        PLUDE(JL, JK, IBL) = PLUDE(JL, JK, IBL)*ZDTGDP
        
        IF (LDCUM(JL, IBL) .and. PLUDE(JL, JK, IBL) > YRECLDP%RLMIN .and. PLU(JL, JK + 1, IBL) > ZEPSEC) THEN
          
          ZSOLAC = ZSOLAC + PLUDE(JL, JK, IBL) / PLU(JL, JK + 1, IBL)
          ! *diagnostic temperature split*
          ZALFAW = ZFOEALFA
          ZCONVSRCE(NCLDQL) = ZALFAW*PLUDE(JL, JK, IBL)
          ZCONVSRCE(NCLDQI) = (1.0_JPRB - ZALFAW)*PLUDE(JL, JK, IBL)
          ZSOLQA(NCLDQL, NCLDQL) = ZSOLQA(NCLDQL, NCLDQL) + ZCONVSRCE(NCLDQL)
          ZSOLQA(NCLDQI, NCLDQI) = ZSOLQA(NCLDQI, NCLDQI) + ZCONVSRCE(NCLDQI)
          
        ELSE
          
          PLUDE(JL, JK, IBL) = 0.0_JPRB
          
        END IF
        ! *convective snow detrainment source
        IF (LDCUM(JL, IBL))           ZSOLQA(NCLDQS, NCLDQS) = ZSOLQA(NCLDQS, NCLDQS) + PSNDE(JL, JK, IBL)*ZDTGDP
      ...
      IF (JK > YRECLDP%NCLDTOP) THEN
        ...
        ZDTDP = ZRDCP*0.5_JPRB*(ZTP1(JK_IM1) + ZTP1(JK_I)) / PAPH(JL, JK, IBL)
        ZDTFORC = ZDTDP*(PAP(JL, JK, IBL) - PAP(JL, JK - 1, IBL))
      ...
      IF (JK < KLEV) THEN
        ...
        ZMFDN = MAX(0.0_JPRB, (PMFU(JL, JK + 1, IBL) + PMFD(JL, JK + 1, IBL))*ZDTGDP)
      ...
      IF (KTYPE(JL, IBL) > 0 .and. PLUDE(JL, JK, IBL) > ZEPSEC)         ZLDIFDT = YRECLDP%RCLDIFF_CONVI*ZLDIFDT
      ...
      ZDTDP = ZRDCP*ZTP1(JK_I) / PAP(JL, JK, IBL)
      ZDPMXDT = ZDP*ZQTMST
      ZMFDN = 0.0_JPRB
      IF (JK < KLEV)         ZMFDN = PMFU(JL, JK + 1, IBL) + PMFD(JL, JK + 1, IBL)
      ZWTOT = PVERVEL(JL, JK, IBL) + 0.5_JPRB*RG*(PMFU(JL, JK, IBL) + PMFD(JL, JK, IBL) + ZMFDN)
      ZWTOT = MIN(ZDPMXDT, MAX(-ZDPMXDT, ZWTOT))
      ZZZDT = PHRSW(JL, JK, IBL) + PHRLW(JL, JK, IBL)
      ...
      ZQP = 1.0_JPRB / PAP(JL, JK, IBL)
      ZQSAT = ((R2ES*((MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2))*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES)) + (1.0_JPRB - (MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2)))*EXP((R3IES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4IES)))))*ZQP
      ZQSAT = MIN(0.5_JPRB, ZQSAT)
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
      ZQSAT = ZQSAT*ZCOR
      ZCOND = (ZQSMIX - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*((((MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2))*R5ALVCP)*(1.0_JPRB/(ZTP1(JK_I) - R4LES)**2) + ((1.0_JPRB - (MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2)))*R5ALSCP)*(1.0_JPRB/(ZTP1(JK_I) - R4IES)**2))))
      ZTP1(JK_I) = ZTP1(JK_I) + (((MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2)))*RALVDCP+(1.0_JPRB-((MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2))))*RALSDCP)*ZCOND
      ZQSMIX = ZQSMIX - ZCOND
      ZQSAT = ((R2ES*((MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2))*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES)) + (1.0_JPRB - (MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2)))*EXP((R3IES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4IES)))))*ZQP
      ZQSAT = MIN(0.5_JPRB, ZQSAT)
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
      ZQSAT = ZQSAT*ZCOR
      ZCOND1 = (ZQSMIX - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*((((MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2))*R5ALVCP)*(1.0_JPRB/(ZTP1(JK_I) - R4LES)**2) + ((1.0_JPRB - (MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2)))*R5ALSCP)*(1.0_JPRB/(ZTP1(JK_I) - R4IES)**2))))
      ZTP1(JK_I) = ZTP1(JK_I) + (((MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2)))*RALVDCP+(1.0_JPRB-((MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,ZTP1(JK_I)))-RTICE)*RTWAT_RTICE_R)**2))))*RALSDCP)*ZCOND1
      ...
      IF (ZA(JK_I) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN
        ...
        IF (ZA(JK_I) > 0.99_JPRB) THEN
          ...
          ZCDMAX =  &
          & (ZQX(NCLDQV) - ZQSMIX) / (1.0_JPRB + ZCOR*ZQSMIX*((((MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2))*R5ALVCP)*(1.0_JPRB/(ZTP1(JK_I) - R4LES)**2) + ((1.0_JPRB - (MIN(1.0_JPRB, ((MAX(RTICE, MIN(RTWAT, ZTP1(JK_I))) - RTICE)*RTWAT_RTICE_R)**2)))*R5ALSCP)*(1.0_JPRB/(ZTP1(JK_I) - R4IES)**2))))
        ELSE
      ...
      IF (IDEPICE == 1) THEN
        ...
        IF (ZTP1(JK_I) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
          ...
          ZVPICE = (((R2ES*EXP((R3IES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4IES))))*RV) / RD
          ...
          ZBDD = RV*ZTP1(JK_I)*PAP(JL, JK, IBL) / (2.21_JPRB*ZVPICE)
      ...
      ELSE IF (IDEPICE == 2) THEN
        ...
        IF (ZTP1(JK_I) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
          ...
          ZAPLUSB = YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JK_I) + PAP(JL, JK, IBL) &
          & *YRECLDP%RCL_APB3*ZTP1(JK_I)**3._JPRB
      ...
      IF (ZQPRETOT > ZEPSEC) THEN
        ZCOVPTOT = 1.0_JPRB - ((1.0_JPRB - ZCOVPTOT)*(1.0_JPRB - MAX(ZA(JK_I), ZA(JK_IM1)))) / (1.0_JPRB -  &
        & MIN(ZA(JK_IM1), 1.0_JPRB - 1.E-06_JPRB)) ! here!!!
      ...
      IF (ZTP1(JK_I) <= RTT) THEN
        ...
        IF (ZICECLD > ZEPSEC) THEN
          ...
          IF (YRECLDP%LAERICEAUTO) THEN
            ZLCRIT = PICRIT_AER(JL, JK, IBL)
            ! 0.3 = N**0.333 with N=0.027
            ZZCO = ZZCO*(YRECLDP%RNICE / PNICE(JL, JK, IBL))**0.333_JPRB
          ELSE
      ...
      IF (ZLIQCLD > ZEPSEC) THEN
        ...
        IF (IWARMRAIN == 1) THEN
          ...
          IF (YRECLDP%LAERLIQAUTOLSP) THEN
            ZLCRIT = PLCRIT_AER(JL, JK, IBL)
            ! 0.3 = N**0.333 with N=125 cm-3
            ZZCO = ZZCO*(YRECLDP%RCCN / PCCN(JL, JK, IBL))**0.333_JPRB
          ELSE
          ...
          IF (YRECLDP%LAERLIQCOLL) THEN
            ...
            ZCFPR = ZCFPR*(YRECLDP%RCCN / PCCN(JL, JK, IBL))**0.333_JPRB
          END IF
      ...
      IF (ZICETOT > ZEPSEC .and. ZTP1(JK_I) > RTT) THEN
        ...
        ! due to evaporation.
        ZTDMTW0 = ZTP1(JK_I) - RTT - ZSUBSAT*(ZTW1 + ZTW2*(PAP(JL, JK, IBL) - ZTW3) - ZTW4*(ZTP1(JK_I) - ZTW5))
        ! Not implicit yet...
      ...
      IF (ZQX(NCLDQR) > ZEPSEC) THEN
        ...
        IF (ZTP1(JK_I) <= RTT .and. ZTP1(JK_IM1) > RTT) THEN
          ...
          PRAINFRAC_TOPRFZ(JL, IBL) = ZQX(NCLDQR) / ZQPRETOT
          IF (PRAINFRAC_TOPRFZ(JL, IBL) > 0.8) THEN
            LLRAINLIQ = .true.
          ELSE
            LLRAINLIQ = .false.
          END IF
      ...
      IF (IEVAPRAIN == 1) THEN
        ...
        IF (LLO1) THEN
          ...
          ZBETA1 = SQRT(PAP(JL, JK, IBL) / PAPH(JL, KLEV + 1, IBL)) / YRECLDP%RVRFACTOR*ZPRECLR / MAX(ZCOVPCLR, ZEPSEC)
      ...
      ELSE IF (IEVAPRAIN == 2) THEN
        ...
        IF (LLO1) THEN
          ...
          ZESATLIQ = RV / RD*((R2ES*EXP((R3LES*(ZTP1(JK_I) - RTT))/(ZTP1(JK_I) - R4LES))))
          
          ! Slope of particle size distribution
          ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*ZPRECLR))**YRECLDP%RCL_FAC2              ! ZPRECLR=kg/kg
          
          ZEVAP_DENOM = YRECLDP%RCL_CDENOM1*ZESATLIQ - YRECLDP%RCL_CDENOM2*ZTP1(JK_I)*ZESATLIQ +  &
          & YRECLDP%RCL_CDENOM3*ZTP1(JK_I)**3._JPRB*PAP(JL, JK, IBL)
      ...
      ELSE IF (IEVAPSNOW == 2) THEN
        ...
        IF (LLO1) THEN
          ...
          !-------------------------------------------------------------
          ZCOVPTOT =  &
          & MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZA(JK_I))*ZEVAP / ZQX(NCLDQS)))
      ...
      DO JM=1,NCLV - 1
        ...
        IF (IPHASE(JM) == 1) THEN
          TENDENCY_LOC(JL, JK, 1, IBL) =  &
          & TENDENCY_LOC(JL, JK, 1, IBL) + RALVDCP*(ZQXN(JM) - ZQX(JM) - ZFLUXQ(JM))*ZQTMST
        END IF
        
        IF (IPHASE(JM) == 2) THEN
          TENDENCY_LOC(JL, JK, 1, IBL) =  &
          & TENDENCY_LOC(JL, JK, 1, IBL) + RALSDCP*(ZQXN(JM) - ZQX(JM) - ZFLUXQ(JM))*ZQTMST
        END IF
        ...
        !----------------------------------------------------------------------
        TENDENCY_LOC(JL, JK, 3 + JM, IBL) = TENDENCY_LOC(JL, JK, 3 + JM, IBL) + (ZQXN(JM) - ZQX0(JM))*ZQTMST
      ...
      ! 6.2 Humidity budget
      !----------------------
      TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + (ZQXN(NCLDQV) - ZQX(NCLDQV))*ZQTMST
      
      !-------------------
      ! 6.3 cloud cover
      !-----------------------
      TENDENCY_LOC(JL, JK, 2, IBL) = TENDENCY_LOC(JL, JK, 2, IBL) + ZDA*ZQTMST
      
      !--------------------------------------------------
      ! Copy precipitation fraction into output variable
      !-------------------------------------------------
      PCOVPTOT(JL, JK, IBL) = ZCOVPTOT
    ...
    !--------------------------------------------------------------------
    PFPLSL(JL, JK, IBL) = ZPFPLSX(JK_I, NCLDQR) + ZPFPLSX(JK_I, NCLDQL)
    PFPLSN(JL, JK, IBL) = ZPFPLSX(JK_I, NCLDQS) + ZPFPLSX(JK_I, NCLDQI)
    
    if (1<=JK .AND. JK<=KLEV) THEN
      
      ZGDPH_R = -ZRG_R*(PAPH(JL, JK + 1, IBL) - PAPH(JL, JK, IBL))*ZQTMST
      PFSQLF(JL, JK + 1, IBL) = PFSQLF(JL, JK, IBL)
      PFSQIF(JL, JK + 1, IBL) = PFSQIF(JL, JK, IBL)
      PFSQRF(JL, JK + 1, IBL) = PFSQLF(JL, JK, IBL)
      PFSQSF(JL, JK + 1, IBL) = PFSQIF(JL, JK, IBL)
      PFCQLNG(JL, JK + 1, IBL) = PFCQLNG(JL, JK, IBL)
      PFCQNNG(JL, JK + 1, IBL) = PFCQNNG(JL, JK, IBL)
      PFCQRNG(JL, JK + 1, IBL) = PFCQLNG(JL, JK, IBL)
      PFCQSNG(JL, JK + 1, IBL) = PFCQNNG(JL, JK, IBL)
      PFSQLTUR(JL, JK + 1, IBL) = PFSQLTUR(JL, JK, IBL)
      PFSQITUR(JL, JK + 1, IBL) = PFSQITUR(JL, JK, IBL)
      
      ZALFAW = ZFOEALFA
      
      ! Liquid , LS scheme minus detrainment
      PFSQLF(JL, JK + 1, IBL) = PFSQLF(JL, JK + 1, IBL) + (ZQXN2D(NCLDQL) - ZQX0(NCLDQL) + PVFL(JL,  &
      & JK, IBL)*PTSPHY - ZALFAW*PLUDE(JL, JK, IBL))*ZGDPH_R
      ! liquid, negative numbers
      PFCQLNG(JL, JK + 1, IBL) = PFCQLNG(JL, JK + 1, IBL) + ZLNEG(NCLDQL)*ZGDPH_R
      
      ! liquid, vertical diffusion
      PFSQLTUR(JL, JK + 1, IBL) = PFSQLTUR(JL, JK + 1, IBL) + PVFL(JL, JK, IBL)*PTSPHY*ZGDPH_R
      
      ! Rain, LS scheme
      PFSQRF(JL, JK + 1, IBL) = PFSQRF(JL, JK + 1, IBL) + (ZQXN2D(NCLDQR) - ZQX0(NCLDQR))*ZGDPH_R
      ! rain, negative numbers
      PFCQRNG(JL, JK + 1, IBL) = PFCQRNG(JL, JK + 1, IBL) + ZLNEG(NCLDQR)*ZGDPH_R
      
      ! Ice , LS scheme minus detrainment
      PFSQIF(JL, JK + 1, IBL) = PFSQIF(JL, JK + 1, IBL) + (ZQXN2D(NCLDQI) - ZQX0(NCLDQI) + PVFI(JL,  &
      & JK, IBL)*PTSPHY - (1.0_JPRB - ZALFAW)*PLUDE(JL, JK, IBL))*ZGDPH_R
      ! ice, negative numbers
      PFCQNNG(JL, JK + 1, IBL) = PFCQNNG(JL, JK + 1, IBL) + ZLNEG(NCLDQI)*ZGDPH_R
      
      ! ice, vertical diffusion
      PFSQITUR(JL, JK + 1, IBL) = PFSQITUR(JL, JK + 1, IBL) + PVFI(JL, JK, IBL)*PTSPHY*ZGDPH_R
      
      ! snow, LS scheme
      PFSQSF(JL, JK + 1, IBL) = PFSQSF(JL, JK + 1, IBL) + (ZQXN2D(NCLDQS) - ZQX0(NCLDQS))*ZGDPH_R
      ! snow, negative numbers
      PFCQSNG(JL, JK + 1, IBL) = PFCQSNG(JL, JK + 1, IBL) + ZLNEG(NCLDQS)*ZGDPH_R
    END IF
    
      !-----------------------------------
      ! enthalpy flux due to precipitation
      !-----------------------------------
      PFHPSL(JL, JK, IBL) = -RLVTT*PFPLSL(JL, JK, IBL)
      PFHPSN(JL, JK, IBL) = -RLSTT*PFPLSN(JL, JK, IBL)
    END DO
    !===============================================================================
...
END MODULE CLOUDSC_STD_PAR_SCC_K_CACHING_MOD
